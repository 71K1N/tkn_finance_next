...
export default function PageCategory() {
    ...
    async function store() {
        const subcategory = { categoryId, name, description }
        let response = await fetch(apiUrl + "/subcategory", {
            method: "POST", 
        ...
        response.status == 201?swal("Salvo!", "Adicionado com sucesso!", "success"):swal("Erro!", "Erro ao salvar, Verifique os dados!", "error");
    ...
    async function remove(id:number) {
        swal({
          ...
          .then(willDelete => {
            if (willDelete) {
                let response = fetch(`${apiUrl}/subcategory/${id}`,{method:'DELETE'}).then(()=>{
    ...
    function handleCategoryDescriptionTable(id:number) {
        const category1:any = categories.filter((category)=>{
                ...
                <div className="card-body">
                    <div className="row">
                        <div className="col-md-4">
                            <div className="form-floating mg-1">
                            
                            <select className="form-select" aria-label="Default select" id="categoryId" value={categoryId} onChange={handleSelect}>
                                ...
                                <tbody>                                    
                                    {
                                        subCategories.map(item => (
                                            <tr key={item.id}>
                                                ...
                                                <td>{handleCategoryDescriptionTable(item.categoryId)}</td>
                                                <td>
